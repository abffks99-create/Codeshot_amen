...
'''
            image_prompt = (
                ...
                "- 안전: 이미지에서 피싱 요소가 전혀 발견되지 않을 때\n"
                "  단, 이미지에 google.com, naver.com, kakao.com, youtube.com 등\n"
                "  공인된 공식 사이트의 정상 화면이 명확히 보이고 피싱 요소가 없으면 안전으로 판정\n"
...
'''
        analysis_prompt = (
            ...
            "safe는 해당 항목이 완전히 검증된 경우에만 사용하고, 불확실하면 warning을 사용하세요.\n\n"
            "⚠️ level(전체 위험도) 판정 기준:\n"
            "- 안전: 아래 두 조건 중 하나를 만족할 때\n"
            "  ① google.com, youtube.com, naver.com, kakao.com, daum.net, samsung.com,\n"
            "    apple.com, microsoft.com, instagram.com, facebook.com, twitter.com,\n"
            "    wikipedia.org, github.com, netflix.com 등 전 세계적으로 공인된 공식 도메인이고\n"
            "    URL 구조가 정상이며(서브도메인 포함) 피싱 요소가 전혀 없을 때\n"
            "  ② 모든 분석 항목에서 위협 요소가 전혀 발견되지 않을 때\n"
            "- 주의: 의심 요소가 일부 있거나 확실하지 않은 경우 (저위험·중위험 모두 포함)\n"
            "- 고위험: 피싱 패턴이 명확히 발견될 때\n"
            "확실히 안전하다고 보장할 수 없으면 반드시 주의 이상으로 판정하세요.\n"
            "단, 위 공인 도메인 목록에 해당하고 URL이 정상이면 저작권 날짜 등 사소한 요소는 무시하세요.\n\n"
            "답변은 반드시 한국어로 작성하고, JSON만 출력하세요.\n"
        )
